...
def show_heatmap(attack_type):
    for network in networks:
        for name in names:
            # Get results
            # adversarial
            path_bagnet9 = results_path+'\\{}\\{}\\heatmap\\BagNet{}\\{}\\{}'
            path_bagnet17 = results_path+'\\{}\\{}\\heatmap\\BagNet{}\\{}\\{}'
            path_bagnet33 = results_path+'\\{}\\{}\\heatmap\\BagNet{}\\{}\\{}'
            print(f"{network} {name}:\n")
            bagnet9_or = np.load(path_bagnet9.format(attack_type,network,'9',name,'original.npy'))
            bagnet17_or = np.load(path_bagnet17.format(attack_type,network,'17',name,'original.npy'))
            bagnet33_or = np.load(path_bagnet33.format(attack_type,network,'33',name,'original.npy'))
            bagnet9_target = np.load(path_bagnet9.format(attack_type,network,'9',name,'target.npy'))
            bagnet17_target = np.load(path_bagnet17.format(attack_type,network,'17',name,'target.npy'))
            bagnet33_target = np.load(path_bagnet33.format(attack_type,network,'33',name,'target.npy'))
            or_, target = class_dict[name][0], class_dict[name][1]
            f, (ax1, ax2, ax3, ax4, ax5, ax6) = plt.subplots(1, 6, sharey=True, figsize=(13,10))

            # ancestor
            ancestor_or = np.load(results_path+'\\EA\\heatmap_BagNet17\\{}\\original.npy'.format(name))
            ancestor_target = np.load(results_path+'\\EA\\heatmap_BagNet17\\{}\\target.npy'.format(name))
            ...
            f.suptitle("{} attack, {} {}".format(attack_type,network,name))
            plt.savefig(analysis_results_path+"\\heatmap\\{}\\{}\\{}.png".format(attack_type,network,name))
            plt.show()


def show_heatmap_quantiles(attack_type,network,name,patch_size):
    '''
    network - string, one of the 10 CNNs
    name - string, one of the 10 ancestor names
    patch_size - int, one of [9,17,33]
    '''
    #texture of ancestors
    ancestor_ca = np.load(results_path+'\\EA\\heatmap_BagNet17\\{}\\original.npy'.format(name))
    ancestor_ct = np.load(results_path+'\\EA\\heatmap_BagNet17\\{}\\target.npy'.format(name))
    #texture of CNN adversarials
    path_bagnet17 = results_path+'\\{}\\{}\\heatmap\\BagNet{}\\{}\\{}'
    BagNeteffect_ca = np.load(path_bagnet17.format(attack_type,network,'17',name,'original.npy'))
    BagNeteffect_ct = np.load(path_bagnet17.format(attack_type,network,'17',name,'target.npy'))

    path = results_path+'\\{}\\{}\\patch_replacement_overlapping\\patch_size{}\\{}\\{}.npy'
    CNNeffect_ca = np.load(path.format(attack_type,network,patch_size,name,'orig'))
    CNNeffect_ct = np.load(path.format(attack_type,network,patch_size,name,'target'))
