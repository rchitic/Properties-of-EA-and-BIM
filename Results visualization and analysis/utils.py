...
def bagnet17(pretrained=True, strides=[2, 2, 2, 1], **kwargs):
    ...
    if pretrained:
        ...
        #model = torch.load(model_zoo.load_url(model_urls['bagnet17']), map_location=torch.device('cpu'))
        #l = torch.load(model_zoo.load_url(model_urls['bagnet17']))
    return model
