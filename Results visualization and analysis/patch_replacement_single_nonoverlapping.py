    ...
    for network in networks:
        for name in names:
            for order in range(1,11):
                ...
                ancestorEA = np.load(path.format('EA',network,patch_size,name,'orig',order))
                targetEA = np.load(path.format('EA',network,patch_size,name,'target',order))
                ancestorBIM = np.load(path.format('BIM',network,patch_size,name,'orig',order))
                targetBIM = np.load(path.format('BIM',network,patch_size,name,'target',order))

                path_original_preds = results_path+"\\EA\\original_preds\\{}\\{}{}.npy".format(network,name,order)
                ...
                f.suptitle("{} {} {} patch size {}".format(network,name,order,patch_size))
            
                ax1.legend(prop={'size': 13})
                ax2.legend(prop={'size': 13})
            
                plt.savefig(analysis_results_path+"\\nonoverlapping single patch replacement\\{}\\{}{}_patchSize{}.png".format(network,name,order,patch_size))
                plt.show()
