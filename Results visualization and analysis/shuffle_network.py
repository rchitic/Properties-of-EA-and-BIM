    ...
    for network in networks:
        for name in names:
            for order in range(1,11):
                print(f"{network} {name} {order}:\n")
                adv = np.load(results_path+'\\{}\\{}\\shuffle_network\\{}\\{}\\preds\\adv_network{}.npy'.format(attack_type,network,str(patch_size),name,order))
...
def show_images(patch_size):
    for network in networks:
        ...
        for name in names:
            for order in range(1,11):
                print(f"{network} {name} {order}:\n")
                adv = np.load(results_path+'\\{}\\{}\\shuffle_network\\{}\\{}\\images\\adv_network{}.npy'.format(attack_type,network,str(patch_size),name,order))
                or_, target = class_dict[name][0], class_dict[name][1]
                plt.imshow(_01(adv))
                plt.show()
